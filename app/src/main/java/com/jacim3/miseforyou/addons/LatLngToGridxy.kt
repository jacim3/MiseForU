...
var TO_GPS = 1         // 위경도 변환
/*
    공공 데이터 - 우리동네정보 Api 를 이용하기 위하여, 위경도 좌표(GPS)를 기상청 격자(Grid)로 변환하는 클래스
*/
class LatLngToGridxy(lat:Double, lng:Double ) {

    var tmp : LatXLngY? = null
    var locX = 0
    var locY = 0
    ...
    init {
        tmp = convertGRID_GPS(TO_GRID, lat, lng)
        locX = tmp!!.x.toInt()
        locY = tmp!!.y.toInt()
    }
}
